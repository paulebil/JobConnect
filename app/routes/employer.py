...
from app.responses.dashboard import ApplicationStatus, EmployerDashboard, ApplicationWithJobSeeker, ApplicantsView
...
def get_employer_company_profile_service(session: AsyncSession = Depends(get_session))-> EmployerCompanyProfileService:
    ...
    return EmployerCompanyProfileService(employer_company_profile_repository,
                                         user_repository, jobseeker_profile_repository,
                                         job_repository, application_repository)


@employer_company_profile_router.post("/profile", status_code=status.HTTP_201_CREATED,
                                      response_model=EmployerCompanyProfileResponse)
async def create_profile( user_id: int = Form(), company_name: str = Form(), company_description: str = Form(),
    company_phone: str = Form(), company_location: str = Form(), profile_pic: UploadFile = File(),
...
@employer_company_profile_router.get("/profile/{user_id}/image", status_code=status.HTTP_200_OK)
async def get_profile_pic(user_id: int,
    employer_company_profile_service: EmployerCompanyProfileService = Depends(get_employer_company_profile_service)):

    return await employer_company_profile_service.get_profile_image(user_id)

@employer_company_profile_router.get("/dashboard", status_code=status.HTTP_200_OK,response_model=EmployerDashboard)
async def get_my_dashboard(user_id: int,
    employer_company_profile_service: EmployerCompanyProfileService = Depends(get_employer_company_profile_service)):
    return await employer_company_profile_service.get_dashboard_information(user_id)


@employer_company_profile_router.get("/applicants", status_code=status.HTTP_200_OK,
                                     response_model=ApplicantsView)
async def get_my_applicants_profiles(user_id: int,
    employer_company_profile_service: EmployerCompanyProfileService = Depends(get_employer_company_profile_service)):
    return await employer_company_profile_service.get_all_profiles_for_my_applications(user_id)

@employer_company_profile_router.get("/applicants/detail/{jobseeker_id}/{job_id}", status_code=status.HTTP_200_OK,
                                     response_model=ApplicationWithJobSeeker)
async def get_applicant_profile_detail(jobseeker_id: int, job_id: int,
    employer_company_profile_service: EmployerCompanyProfileService = Depends(get_employer_company_profile_service)):
    return await employer_company_profile_service.get_single_application_jobseeker_profile_detail(jobseeker_id, job_id)


@employer_company_profile_router.patch("/applicants/update/{jobseeker_id}/{job_id}", status_code=status.HTTP_200_OK,
                                       response_model=ApplicationWithJobSeeker)
async def get_applicant_profile_detail(jobseeker_id: int, job_id: int, app_status:ApplicationStatus ,
    employer_company_profile_service: EmployerCompanyProfileService = Depends(get_employer_company_profile_service)):
